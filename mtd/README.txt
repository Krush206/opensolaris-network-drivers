...
This is the Myson mtd803 fast ethernt NIC driver for Solaris.
It is distributed under the BSD license.

2. Specification of the driver
File name of the driver: /kernel/drv/mtd
Special file name: /dev/mtdN (Where N is a unit number, typcally 0 for first
card)
        For example
                % ifconfig mtd0
...
(2) Copy source and binary files.
        # gunzip -cd mtd-x.x.x.tar.gz | tar xf -
...
You can enjoy the new functions by recompiling mtd source code.
Before you compile the source, you need to prepare OpenSolaris
source code and modify ONUTSDIR macro in Makefile.config_gld3
to point the source code directory.
...
4. Testing
  Testing before installation is strongly recommended.
	...
	# init 6	(reboot the system)
        ...
        # modload obj/mtd
        # devfsadm -i mtd  (for solaris7, use drvconfig and reboot with -r )
        # ifconfig mtdN plumb ( where N is an instance number, typcally 0 for first card)
        # ifconfig -a        ( you will see an entry for mtdN)
        # ifconfig mtdN YOUR-HOST-NAME
        # ifconfig mtdN      ( ensure IP address is correct)
        # ifconfig mtdN up   ( and then you can test with ping, telnet, ftp ...)

5. Installation
    After you ensure that the mtd driver is fully functional, install it.

(1) install the mtd driver into the kernel directory
        # cd /.../mtd-x.x.x
        # /usr/ccs/bin/make install

    If you do not test the mtd driver yet, execute the following commands:
            # ./adddrv.sh
	    # init 6          (reboot the system)
            # devfsadm -i mtd (for solaris7, use drvconfig and reboot with -r)
...
Q. Following message is printed in the /var/adm/messages file
    WARNING: mtdN: link up but auto-nego failed, it's funny.
...
A. This massage may appear when the link partner does not have auto
  ...
  the correct media mode in /kernel/drv/mtd.conf according to the following
  syntax:

   mtdN-duplex=["full"|"half"] mtdN-speed=[100|10]; # where N is a unit number

  For example:
        mtd0-duplex="full" mtd0-speed=100;   # full-duplex 100Mbps for mtd0
        mtd0-duplex="half" mtd0-speed=10;    # half-duplex 10Mbps for mtd0
...
A. Uninstall the NIC card and reboot solaris. Then, unregister the driver.
   ...
   Or boot solaris with -a option and use /etc/system.nomtd instead of
   default [etc/system] to inhibit loading the driver.

   /etc/system.nomtd is automatically generated when executing 'make install'

Q. Solaris boots, but the network interface does not work.
A.
   Is the network interface detected?

   Execute 'ifconfig -a'.  If you see an entry for mtdN, the driver is loaded
