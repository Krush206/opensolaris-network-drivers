...
1. Introduction
  ...
  This is Davicom dm9601 usb1.x fast ethernet driver for Solaris.
...
File name of the driver:
	/kernel/drv/udmf
	/kernel/drv/sparcv9/udmf (for sparc 64bit kernel)

Special file name:
	/dev/udmfN (where N is a unit number, typcally 0 for first device)
        For example
                % ifconfig udmf0

Tested OS version
 solaris express nv 114
 As udmf driver requires new usb framework of Solaris, it doesn't work under
 solaris9 or earlier.

Tested chips and usb devices
 DM9601E, Corega K. K.  FEther TXC

3. Preparing for installation

(1) extract distributed source and binary files.
        # gunzip -cd udmf-x.x.x.tar.gz | tar xf -

(2) Add hostname for the NIC device into the /etc/hosts file

(3) Making binaries (required for sparc platforms only)
If you want to test or install udmf on sparc, you need to make
...
4. Installation
 ...
 (2) register the udmf driver with solaris.
 ...
 (4) set up the network port 
        # ifconfig udmfN plumb ( where N is unit number, typcally 0 for first device)
        # ifconfig -a        ( you will see an entry for udmf0, ensure ethernet address is correct)
        # ifconfig udmfN HOSTNAME
        # ifconfig udmfN      ( ensure IP address is correct)
        # ifconfig udmfN up   ( and then you can test with ping, telnet, ftp ...)
...
Q. Following message is printed in the /var/adm/messages file
    NOTE: udmfN: link up but auto-negotiation failed.
...
A. This massage may appear when the link partner does not have auto
   ...
   the correct media mode in /kernel/drv/udmf.conf with the following syntax:
...
A. Is the network interface running?
   ...
   Execute 'ifconfig -a'.  If you see an entry for udmfN, the driver is loaded
