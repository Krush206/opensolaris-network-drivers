...
This is the DP83820 GbE NIC driver of Solaris.
It is distributed under the BSD license.


2. Specification of the driver

File name of the driver: /kernel/drv/nsg

Special file name:
	/dev/nsgN (where N is a unit number, typcally 0 for first card)
...
(2) If previous version of nsg driver is installed, uninstall it first
    and reboot the system.
	# rem_drv nsg	(ignore error messages)
	# init 6

(3) Copy source and binary files.
        # gunzip -cd nsg-x.x.x.tar.gz | tar xf -
...
4. Testing
	...
	# modload obj/nsg
	# devfsadm -i nsg (for solaris7, use drvconfig and then reboot with -r )
	# ifconfig nsgN plumb
	# ifconfig -a	    (you will see an entry for nsgN, ensure MAC address)
	# ifconfig nsgN YOUR-HOSTNAME
	# ifconfig nsgN	    (ensure IP address is correct)
	# ifconfig nsgN up  (and then you can test with ping, telnet, ftp ...)
...
5. Installation
    It is recommended to ensure that nsg driver works for your box before
    installing it.

(1) install nsg driver into the kernel directory
	# cd /.../nsg-x.x.x
        # /usr/ccs/bin/make install

    If you do not test nsg driver yet, execute following commands.
	    # ./adddrv.sh
	    # devfsadm -i nsg (for solaris7, use drvconfig and reboot with -r )

(2) Configure the network interface. Create and/or modify the following file.
	/etc/hostname.nsgN	(N is an unit number)
...
(1) install new nsg driver into the kernel directory
	# cd /.../nsg-x.x.x
        # /usr/ccs/bin/make install
...
A. Uninstall the nic card and reboot solaris. Then, unregister the driver.
   ...
   Or boot solaris with -a option and use /etc/system.nonsg instead of
   default [etc/system] to inhibit loading the driver.

   /etc/system.nonsg will be automatically generated while executing 'make
   install'

Q. Solaris booted, but the network interface didn't works.
A. Is the network interface detected?

   Execute 'ifconfig -a'.  If you see an entry for nsgN, the driver is loaded
