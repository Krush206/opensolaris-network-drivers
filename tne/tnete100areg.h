 ...
 */
struct frag {
	volatile uint32_t	DataCount;
	volatile uint32_t	DataAddress;
};
#define	DATACOUNT_LAST	0x80000000	/* in datacount field of struct frag */

#ifndef TNE_MAXRXFRAGS
#  define TNE_MAXRXFRAGS	10
#endif
struct rx_list {
	volatile uint32_t	forward_ptr;
	volatile uint32_t	rx_cstat;
...
#ifndef TNE_MAXTXFRAGS
#  define TNE_MAXTXFRAGS	10
#endif
struct tx_list {
	volatile uint32_t	forward_ptr;
	volatile uint32_t	tx_cstat;
	struct frag	frags[TNE_MAXRXFRAGS];
};
