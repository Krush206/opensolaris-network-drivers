...
Special file name: /dev/mykN (Where N is a unit number, typcally 0 for first
card)
        For example
                % ifconfig myk0
...
Tested OS version
 Solaris nv 83a (Athlon 2800+ w/ SiS760GX)

Tested cards
  88E8053 -- Kurotoshiko GBE-PCIe
...
(2) Copy source and binary files.
        # gunzip -cd myk-x.x.x.tar.gz | tar xf -
...
Myk driver is ready for 64bit and 32bit solaris8 10/00 sparc or
...
4. Testing
  Testing before installation is strongly recommended.
        ...
        # ifconfig mykN plumb ( where N is an instance number, typcally 0 for first card)
        # ifconfig -a        ( you will see an entry for mykN)
        # ifconfig mykN YOUR-HOST-NAME
        # ifconfig mykN      ( ensure IP address is correct)
        # ifconfig mykN up   ( and then you can test with ping, telnet, ftp ...)
...
Q. Following message is printed in the /var/adm/messages file
    WARNING: mykN: ... but the link partner doesn't seem to have
...
A. This massage may appear when the link partner does not have auto
  ...
  the correct media mode in /kernel/drv/myk.conf according to the following
  syntax:
...
A. Uninstall the NIC card and reboot solaris. Then, unregister the driver.
   ...
   Or boot solaris with -a option and use /etc/system.nomyk instead of
   default [etc/system] to inhibit loading the driver.

   /etc/system.nomyk is automatically generated when executing 'make install'

Q. Solaris boots, but the network interface does not work.
A.
   Is the network interface detected?

   Execute 'ifconfig -a'.  If you see an entry for mykN, the driver is loaded
